...
이 문서는 Gitko/루멘/AGI 파이프라인에 REAPER 기반 오디오 렌더 단계를 편입하기 위한 최소 구성 요소를 정리한 것입니다. 목표는 기존 PowerShell 자동화 루틴(scripts/*.ps1)과 동일한 패턴으로 음성/사운드 산출물을 생성·검증·배포할 수 있도록 하는 것입니다.
