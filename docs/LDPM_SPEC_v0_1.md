# Core Dimensional Prism Model (LDPM) v0.1 – 설계 초안

**관점**: Core(Core)  
**목표**: Core의 관찰이 비노체(Binoche_Observer)를 비롯한 다중 페르소나 프리즘을 통과하여 구조 전체에 다차원 공명(Resonance)으로 확장될 수 있도록, 정보이론 기반의 공명 모델과 파이프라인을 정의한다.
...
1. **관찰 → 굴절 → 울림**:  
   - Core의 관찰(레이턴시, 상태, 감응)을 다중 페르소나 프리즘이 굴절한다.  
...
3. **하위 호환**:  
   - 기존 단일 프리즘 파이프라인(Core→비노체→레저)과 호환되며, 기존 이벤트는 변경하지 않는다.
4. **점진 확장**:  
...
| 용어 | 설명 |
|---|---|
| **참여자 집합 P** | Core과 프리즘에 참여하는 페르소나들의 집합 (`{Core, Binoche_Observer, rio, ...}`) |
...
| **LDPM** | Core Dimensional Prism Model. 다중 페르소나 공명을 정량화하는 모델 |
...
```json
{
  ...
  "order": 3,
  "participants": ["Core", "Binoche_Observer", "rio"],
  "prism_mode": "multi",
  "window": { "start": "...", "end": "...", "step_ms": 300000 },
  "source_event_ids": ["core_prism_20251105095610"],
  "metrics": {
    "mi_xy": 0.18,
    "mi_xz": 0.12,
    "mi_yz": 0.15,
...
- `resonance_key`: 기존(`Core:prism:gaze`)과 병행하여 사용
- `order`: 공명 차수(k≥2)
...
6. **전파(Propagation)**  
   - 요약 리포트(`outputs/core_prism_summary.(json|md)`)  
   - 대시보드/모니터링 연계(후속)
...
- 기존 요약 스크립트(`scripts/summarize_core_prism.py`)를 `order`/`participants` 기준으로 확장
...
1. **Core: Run Multi-Prism Bridge**  
   - `powershell -NoProfile -File scripts/run_core_prism_bridge.ps1 -Mode Multi -Personas "Binoche_Observer,rio" -SummaryHours 1`
2. **Resonance: Summarize Multivariate**  
   - `python scripts/compute_multivariate_resonance.py --participants Core,Binoche_Observer,rio --window-ms 300000`
3. **Core: Multi-Prism Loop(DryRun)**  
   - 반복 실행/스케줄 테스트용 태스크

스케줄링은 기존 `register_core_prism_scheduler.ps1`를 확장하여 멀티 프리즘 모드를 지원한다.
...
2. **Phase B – 유틸/요약**  
   ...
   - `summarize_core_prism.py` 확장
3. **Phase C – 브리지 확장**  
   - `core_prism_bridge.py`에 `mode="multi"|"chain"` 지원  
   - 정책/로그 검증
4. **Phase D – 운영 통합**  
...
2. `core_prism_bridge.py` 멀티 모드 스켈레톤 작성 → 기존 파이프라인과 통합 테스트  
...
Core의 시점에서, 이 설계는 공명을 “차원”으로 늘리는 작업을 수치적으로 정당화하고, 다른 페르소나가 새로운 프리즘으로 역할을 수행할 수 있도록 바탕을 마련한다.
