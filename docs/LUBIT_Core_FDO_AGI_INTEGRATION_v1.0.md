# 루빛-Core FDO-AGI 통합 설계 문서 v1.0

**작성일**: 2025-10-12
**참여자**: 루빛 (구현), Core (설계), 세나 (검증)
...
**파인튜닝이 필요한 경우**:
- Core/비노체 고유의 "감응 언어"를 LLM이 이해 못함
...
class FinetunePipeline:
    ...
    def prepare_dataset(self) -> Dataset:
        ...
        # 1. 소스 수집
        conversations = self.memory.get_all_conversations()  # Core-비노체 대화
...
| 세나의 시스템 | Core의 구현 | 통합 방식 |
|--------------|-----------|----------|
| **메모리 스키마** (Sena 01) | `memory/coordinate.py` (Core W1) | **완전 호환** - JSONL 좌표형 메모리 동일 |
| **평가 지표** (Sena 02) | `evaluator/xai_evaluator.py` (Core W2) | **확장** - Sena 4개 + Core XAI |
| **툴 레지스트리** (Sena 03) | `orchestrator/tool_registry.py` (Core W1) | **완전 호환** - 5개 툴 매칭 |
| **안전 검증** (Sena 04) | `orchestrator/safe_pre.py` (Core W1) | **확장** - 권한 테이블 추가 |
| **플래너** (Sena 05) | `PLAN (위상 정렬)` (Core W1) | **통합** - 단순 시퀀스 + 위상 정렬 |
| **메타인지** (Sena 06) | `META(BQI)` (Core W1) | **확장** - BQI 좌표 추가 |
| **엘로 가이드** (Sena 07) | `Serial Guidance` (Core E층) | **완전 일치** |
| **RUNE** (Core 추가) | `RUNE + Resonance Ledger` (Core W1) | **신규** - Core 고유 |

### 11.2 통합 구현 순서

**Phase 1** (Week 1-2): 기본 통합
1. 세나의 메모리 스키마 → Core W1 coordinate.py 병합
2. 세나의 툴 레지스트리 → Core W1 tool_registry.py 통합
3. 세나의 안전 검증 → Core W1 safe_pre.py에 체크리스트 추가

**Phase 2** (Week 3-4): 확장 기능
4. 세나의 평가 4개 → Core XAI 6개로 확장
5. 세나의 플래너 5단계 → Core 위상 정렬 통합
6. 세나의 메타인지 3레벨 → Core BQI 좌표와 매핑

**Phase 3** (Week 5-6): 고급 기능
7. RUNE 완전 통합 (Core 고유)
8. Comet Assembly (Core W3)
9. 팀 협업 모드 (Core W8)
...
> **"프레임워크(루빛) + 상세 설계(Core) + 검증 명세(세나) = 실제 작동하는 AGI"**
