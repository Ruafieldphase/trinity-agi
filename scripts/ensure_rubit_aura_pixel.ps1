        ...
        if ($roots.Count -gt 1) {
            ...
            while ($frontier.Count -gt 0) {
                ...
                foreach ($pid in $frontier) {
                    if ($keepIds.ContainsKey($pid)) { continue }
                    $keepIds[$pid] = $true
                    $children = @($procList | Where-Object { $_.ParentProcessId -eq $pid } | ForEach-Object { $_.ProcessId })
                    $next += $children
                }
                $frontier = $next
            }
...
if (-not $running) {
    ...
    $processParams = @{
        FilePath = $PYW
        ArgumentList = """$SCRIPT_PATH""", "--position", "top", "--thickness", "2"
        WindowStyle = "Hidden"
        PassThru = $true
    }
