...
$resolved = if (-not [string]::IsNullOrWhiteSpace($Url)) { $Url } elseif (-not [string]::IsNullOrWhiteSpace($env:LUMEN_GATEWAY_URL)) { $env:LUMEN_GATEWAY_URL } else { 'https://lumen-gateway-x4qvsargwa-uc.a.run.app/chat' }
...
for ($attempt = 1; $attempt -le [Math]::Max(1, $Attempts); $attempt++) {
    ...
    try {
        # Prepare request
        $defaultHeaders = @{ 'Accept' = 'application/json'; 'User-Agent' = 'LumenProbeCollector/1.0' }
...
if ($WriteLatest) {
    try {
        $latestJson = Join-Path $workspace 'outputs/lumen_probe_latest.json'
        $latestMd = Join-Path $workspace 'outputs/lumen_probe_latest.md'
        ...
        $md = @()
        $md += '[LUMEN PROBE DIGEST - ASCII SAFE]'
        $md += ''
        ...
        $md += '---'
        $md += ''
        $md += 'Append-only log: outputs/lumen_probe_log.jsonl'
