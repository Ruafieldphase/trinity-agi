<#
.SYNOPSIS
    Scans a directory (recursively) and builds an index (JSON + Markdown).
.DESCRIPTION
    This script traverses the $Root directory, collects file metadata (size, time, basic tags),
    and saves the result to $OutDir/original_data_index.json and .md.
    It is resilient to long paths and mostly read-only.
.PARAMETER Root
    The root directory to scan. Defaults to C:\workspace\original_data.
.PARAMETER OutDir
    The directory where the output index files will be saved.
.PARAMETER MaxFiles
    Maximum number of files to index (safety limit). Default 10000.
.PARAMETER IncludeExt
    List of file extensions to include in the index.
.PARAMETER ComputeHash
    If set, computes SHA1 hash for files smaller than MaxHashMB.
.PARAMETER MaxHashMB
    The size limit (in MB) for file hashing. Files larger than this are skipped for hashing to save time.
.PARAMETER AllowEmpty
    If set, allows the script to succeed even if the Root directory is missing (generates an empty index).
.PARAMETER OpenMd
    If set, attempts to open the generated Markdown report after completion.
.PARAMETER NoOpen
    If set, suppresses opening the report even if OpenMd is implied.
#>
param(
...
function Get-FileHashSafe([System.IO.FileInfo]$file) {
    <#
    .SYNOPSIS
        Safely computes file hash, respecting size limits and handling errors.
    .DESCRIPTION
        Returns SHA1 hash string or $null if file is too large or inaccessible.
        Uses try-catch to prevent script interruption on file access errors.
    #>
    try {
