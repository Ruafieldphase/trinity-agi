...
def to_markdown(summary: Dict[str, Any], source: str) -> str:
    ...
    md = []
    md.append(f"# Core Latency Report")
    md.append("")
...
def main():
    ap = argparse.ArgumentParser(description='Summarize Core latency JSONL into Markdown and JSON.')
    ap.add_argument('--input', '-i', default='outputs/core_probe_history.jsonl', help='Input JSONL path')
    ap.add_argument('--out-md', default='outputs/core_latency_latest.md', help='Output Markdown path')
    ap.add_argument('--out-json', default='outputs/core_latency_summary.json', help='Output JSON summary path')
