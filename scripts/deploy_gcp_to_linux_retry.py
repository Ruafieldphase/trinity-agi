#!/usr/bin/env python3
"""
Deploy GCP Credentials and Scripts to Linux (Retry)
===================================================
...
Includes retry logic for SFTP transfers.
"""
import os
import sys
import time
...
def sftp_put_with_retry(sftp, local_path, remote_path, retries=3):
    ...
    for i in range(retries):
        try:
            ...
            if remote_size == local_size:
                ...
                print(f"      ⚠️ Size mismatch (Local: {local_size}, Remote: {remote_size}). Retrying...")
                
        except Exception as e:
            print(f"      ⚠️ Upload failed: {e}. Retrying ({i+1}/{retries})...")
            time.sleep(1)
            
    return False

def deploy():
    print("🚀 Starting GCP Deployment to Linux (Robust Mode)...")
    ...
    try:
        ...
        if LOCAL_CREDS_PATH.exists():
            if sftp_put_with_retry(sftp, LOCAL_CREDS_PATH, REMOTE_CREDS_PATH):
                print(f"   ✅ Uploaded to {REMOTE_CREDS_PATH}")
            else:
                print(f"   ❌ Failed to upload credentials after retries")
                return
        else:
