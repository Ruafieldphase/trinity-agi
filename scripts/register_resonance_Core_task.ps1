# Resonance Loop + Core 자동화 등록 스크립트
# 일일 자동 분석을 예약합니다
...
$TaskName = "AGI_ResonanceCoreIntegration"
...
if ($Status -or (-not $Register -and -not $Unregister)) {
    ...
    Write-Host "`n📋 Resonance Loop + Core 자동화 상태`n" -ForegroundColor Cyan
    ...
    if ($task) {
        ...
        Write-Host "`n   등록 명령: .\register_resonance_core_task.ps1 -Register`n" -ForegroundColor Yellow
    }
...
# Unregister
if ($Unregister) {
    Write-Host "`n🗑️ Resonance Loop + Core 자동화 제거`n" -ForegroundColor Yellow
...
# Register
if ($Register) {
    Write-Host "`n📅 Resonance Loop + Core 자동화 등록`n" -ForegroundColor Cyan
    Write-Host "════════════════════════════════════════════════════════════`n" -ForegroundColor Gray
    ...
    # 스크립트 경로
    $scriptPath = Join-Path $WorkspaceRoot "scripts\run_resonance_core_integration.ps1"
...
$settings.Hidden = $true
    ...
    Register-ScheduledTask `
        ...
        -Principal $principal `
        -Settings $settings `
        -Description "AGI Resonance Loop + Core 통합 자동 분석" | Out-Null
    ...
    Write-Host "   .\register_resonance_core_task.ps1`n" -ForegroundColor Cyan
}