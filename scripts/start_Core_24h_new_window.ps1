# Core 24h Production - 새 터미널 창에서 실행
# 안정적이고 출력도 직접 확인 가능
...
Write-Host "║  Core 24h Production 시작                    ║" -ForegroundColor Cyan
Write-Host "╚═══════════════════════════════════════════════╝`n" -ForegroundColor Cyan

$pythonExe = "$WorkspaceRoot\fdo_agi_repo\.venv\Scripts\python.exe"
$scriptPath = "$WorkspaceRoot\fdo_agi_repo\scripts\core_production_24h_stable.py"
...
$command = "cd $WorkspaceRoot\fdo_agi_repo; & '$pythonExe' '$scriptPath'; Read-Host 'Press Enter to close'"
...
Write-Host "   2. 로그 파일: fdo_agi_repo\outputs\core_production_24h_stable.jsonl" -ForegroundColor White
Write-Host "   3. 모니터링 스크립트: .\scripts\monitor_core_24h_simple.ps1" -ForegroundColor White
