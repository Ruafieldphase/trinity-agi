...
function Show-Dashboard {
    ...
    # 3. Core Gateway
    Write-ColorLine "?뱧 梨꾨꼸 3: Core Gateway" "White"
    Write-ColorLine "   ?붾뱶?ъ씤?? https://Core-gateway-x4qvsargwa-uc.a.run.app/chat" "DarkGray"
    ...
    Write-ColorLine "   ?묐떟?쒓컙: $($CoreStatus.ResponseTime)ms" "DarkGray"
    
    if ($CoreStatus.StatusCode -ne 200) {
        Write-ColorLine "   ?ㅻ쪟: $($CoreStatus.ErrorMessage)" "Red"
    }
    ...
    $allGreen = ($proxyRunning -and $localStatus.Status -eq "Online" -and $chatTest.Status -eq "Online" -and 
        $cloudStatus.Status -eq "Online" -and $CoreStatus.Status -eq "Online")
    ...
    Write-ColorLine "   [4] Python ?뚯뒪??  .\.venv\Scripts\python.exe .\test_core_connection.py" "White"
