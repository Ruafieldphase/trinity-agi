...
def demo_cross_layer_sharing():
    ...
    # Scenario: User talks to Sian about Alpha
    print("\n📍 Step 1: User → Sian (대화 레이어)")
    ...
    # Scenario: User switches to Lumen
    print("\n📍 Step 2: User → Lumen (안정화 레이어)")
    print("비노체님: '지금 Alpha 상태가 어때?'")
    
    # Lumen automatically recalls Alpha context
    print("\n🔍 Lumen이 자동으로 Alpha 맥락 검색:")
    ...
    # Lumen already knows about Alpha from Sian's conversation
    ctx4 = Context.create(
        layer="lumen",
        speaker="lumen",
        content="Alpha는 현재 SILENT_OBSERVER 상태입니다. Drift Score는 0.23으로 안정적입니다. Sian과의 대화에서 설명드린 대로, 평소처럼 관찰 모드입니다.",
    ...
    bridge.link_contexts(ctx3.id, ctx2.id)  # Connect to Sian's explanation
    ...
    ctx5 = Context.create(
        layer="rhythm",
        speaker="rhythm",
        content="네, 과거 Panic 상태에서 INTERVENTION이 발동된 기록이 있습니다. Lumen이 방금 전에 현재는 SILENT_OBSERVER라고 보고했습니다.",
    ...
    print("💡 핵심 인사이트:")
    print("=" * 60)
    print("✅ Sian이 설명한 'Alpha'를 Lumen이 자동으로 알고 있음")
    print("✅ Lumen의 상태 보고를 Rhythm이 자동으로 참조함")
