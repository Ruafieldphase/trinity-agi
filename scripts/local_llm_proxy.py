...
"""
로컬 LLM 프록시 서버
http://localhost:8080/v1/chat/completions 요청을 Lumen Gateway로 포워딩
"""
...
LUMEN_GATEWAY_URL = os.getenv(
    "LUMEN_GATEWAY_URL",
    "https://lumen-gateway-x4qvsargwa-uc.a.run.app/chat",
)
...
def health():
    ...
    return jsonify({
        ...
        "forwarding_to": LUMEN_GATEWAY_URL
    })
...
def chat_completions():
    ...
    Lumen Gateway로 변환 후 포워딩
    """
    try:
        data = request.get_json()
        logger.info(f"Received request: {data}")
        
        # OpenAI 형식 -> Lumen 형식 변환
        messages = data.get('messages', [])
        last_message = messages[-1].get('content', '') if messages else ''
        
        lumen_payload = {"message": last_message}
        
        # Lumen Gateway 호출
        # Fast-route: small prompts can go directly to Cloud AI for lower latency
        target_url = LUMEN_GATEWAY_URL
        ...
        if response.status_code == 200:
            lumen_data = response.json()
            
            # Lumen 응답 -> OpenAI 형식 변환
            import time
            openai_response = {
                ...
                "choices": [{
                    ...
                    "message": {
                        "role": "assistant",
                        "content": lumen_data.get('response', '')
                    },
                    "finish_reason": "stop"
                }],
                "usage": {
                    "prompt_tokens": len(last_message) // 4,
                    "completion_tokens": len(lumen_data.get('response', '')) // 4,
                    "total_tokens": (len(last_message) + len(lumen_data.get('response', ''))) // 4
                },
                "lumen_metadata": {
                    "persona": lumen_data.get('persona'),
                    "sources": lumen_data.get('sources'),
                    "timestamp": lumen_data.get('timestamp')
                }
            }
            
            logger.info(f"Success: Persona={lumen_data.get('persona', {}).get('name')}")
            return jsonify(openai_response)
        else:
            logger.error(f"Lumen Gateway error: {response.status_code}")
            return jsonify({
                "error": {
                    "message": f"Lumen Gateway returned {response.status_code}",
                    "type": "gateway_error",
...
if __name__ == '__main__':
    ...
    logger.info(f"Forwarding to: {LUMEN_GATEWAY_URL}")
