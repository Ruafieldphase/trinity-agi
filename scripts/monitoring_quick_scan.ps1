...
Write-Host "=== Lumen Probe (latest) ===" -ForegroundColor Cyan
& (Join-Path $root 'scripts/run_lumen_probe.ps1') -TimeoutSec $ProbeTimeoutSec -Attempts $ProbeAttempts -BackoffMs $ProbeBackoffMs -Tag 'quick-scan' -OpenLatest

Write-Host "`n=== Lumen Probe Summary (${SummaryHours}h) ===" -ForegroundColor Cyan
& (Join-Path $root 'scripts/generate_lumen_probe_summary.ps1') -Hours $SummaryHours -OpenMd
