...
Write-Info "🔎 Safe Migration (non-destructive)"
...
foreach ($f in $Folders) {
    ...
    if (-not (Test-Path -LiteralPath $s)) { Write-Warn "⚠️  Skip missing folder: $s"; continue }
...
if ($conflicts.Count -gt 0) {
    ...
    Write-Warn "⚠️  Conflicts detected: $($conflicts.Count)"
    Write-Host "📄 Conflict report: $ReportPath"
}

Write-Host "\n📈 Summary:" -ForegroundColor Cyan
$stats | Format-Table -AutoSize

Write-Ok "\n✅ Safe migration completed. $(if($DryRun){'No files copied.'} else {'Missing files copied.'})"
exit 0