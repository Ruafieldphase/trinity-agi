...
try {
    ...
    if ($LASTEXITCODE -eq 0 -and $pyOut) {
        try { $ev = $pyOut | ConvertFrom-Json -ErrorAction Stop; if ($ev.min_quality -ne $null) { $evalMinQ = [double]$ev.min_quality } } catch {}
    }
} catch {}
...
if ($evalMinQ -ne $null) { Write-Host ("  Eval min_quality    : {0}" -f $evalMinQ) }

if ($ShowLedger) {
    try {
        $ledger = Join-Path (Split-Path (Resolve-Path ".").Path -ChildPath) "fdo_agi_repo\memory\resonance_ledger.jsonl"
