#!/usr/bin/env python3
"""
Auto-Stabilizer: Core 감정 신호 기반 자동 안정화 시스템
...
def read_core_state() -> Optional[Dict[str, Any]]:
    """Core 상태 파일 읽기"""
    try:
        if not CORE_STATE_PATH.exists():
            log_message(f"Core state file not found: {CORE_STATE_PATH}", "WARN")
            return None
        
        with open(CORE_STATE_PATH, "r", encoding="utf-8-sig") as f:
            state = json.load(f)
        
        return state
    except Exception as e:
        log_message(f"Error reading Core state: {e}", "ERROR")
        return None
...
def get_fear_signal(state: Dict[str, Any]) -> float:
    """Fear 신호 추출"""
    try:
        # Core state structure: {"emotion": {"fear": 0.0-1.0}}
    ...
    try:
        while True:
            ...
            if state:
                ...
                log_message("  Unable to read Core state, skipping evaluation", "WARN")
...
def main():
    parser = argparse.ArgumentParser(description="Auto-Stabilizer: Core 감정 신호 기반 자동 안정화")
    ...
    if args.once:
        ...
        if state:
            ...
            log_message("Unable to read Core state", "ERROR")
            sys.exit(1)
    else:
