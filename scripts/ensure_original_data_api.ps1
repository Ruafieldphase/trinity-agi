...
function Start-OriginalDataApi {
    ...
    $apiArgs = "`"$ServerScript`" --port $Port"
    Write-Info "Starting Original Data API on port $Port using: $py $apiArgs"
    try {
        Start-Process -FilePath $py -ArgumentList $apiArgs -WorkingDirectory $WorkspaceRoot -WindowStyle Hidden | Out-Null
