...
function Get-AGIMetricsFromPython {
    ...
    try {
        # Python ?¤í¬ë¦½íŠ¸ ?¤í–‰?˜ì—¬ JSON ì¶œë ¥ ë°›ê¸°
        ...
        # UTF-8 ?¸ì½”???¤ì • (Windows CP949 ë¬¸ì œ ?´ê²°)
        $originalEncoding = $env:PYTHONIOENCODING
        $env:PYTHONIOENCODING = 'utf-8'
        
        try {
            # stdoutê³?stderr ë¶„ë¦¬?˜ì—¬ JSON ì¶œë ¥ë§?ìº¡ì²˜
            $output = & $PythonExe $DashboardScript --hours $Hours --json 2>&1
            $exitCode = $LASTEXITCODE
            
            # ë¬¸ìž?´ë§Œ ?„í„°ë§í•˜ê³?JSON ?œìž‘?˜ëŠ” ì²?ì¤?ì°¾ê¸°
            $jsonLines = @()
            $inJson = $false
...
if ($agiMetrics.TotalEvents -eq 0) {
    ...
    if ($agiMetrics.AvgQuality -ne $null) {
        $qualityStatus = if ($agiMetrics.AvgQuality -ge 0.8) { "?Ÿ¢" } elseif ($agiMetrics.AvgQuality -ge 0.7) { "?Ÿ¡" } else { "?”´" }
        $reportLines += "| Avg Quality | $qualityStatus $($agiMetrics.AvgQuality) |"
    }
    if ($agiMetrics.SuccessRate -ne $null) {
        $successStatus = if ($agiMetrics.SuccessRate -ge 80) { "?Ÿ¢" } elseif ($agiMetrics.SuccessRate -ge 70) { "?Ÿ¡" } else { "?”´" }
...
if ($agiMetrics.TotalEvents -gt 0) {
    ...
    if ($null -ne $agiMetrics.AvgQuality -and $agiMetrics.AvgQuality -lt $minQuality) {
        $severity = if ($agiMetrics.AvgQuality -lt $critQuality) { "?”´ CRITICAL" } else { "?Ÿ¡ WARNING" }
    ...
    if ($null -ne $agiMetrics.SuccessRate -and $agiMetrics.SuccessRate -lt $minSuccess) {
        $severity = if ($agiMetrics.SuccessRate -lt $critSuccess) { "?”´ CRITICAL" } else { "?Ÿ¡ WARNING" }
    ...
    if ($null -ne $agiMetrics.ReplanRate -and $agiMetrics.ReplanRate -gt $replanThresh) {
        $severity = if ($agiMetrics.ReplanRate -gt $replanCrit) { "?”´ CRITICAL" } else { "?Ÿ¡ WARNING" }
    ...
    if ($null -ne $avgDurToUse -and $avgDurToUse -gt $maxAvgDur) {
        $severity = if ($avgDurToUse -gt $critAvgDur) { "?”´ CRITICAL" } else { "?Ÿ¡ WARNING" }
    ...
    if ($agiMetrics.LastActivity) {
        ...
        if ($timeSince.TotalHours -gt $inactiveHours) {
            $severity = if ($timeSince.TotalHours -gt ($inactiveHours * 3)) { "?”´ CRITICAL" } else { "?Ÿ¡ WARNING" }
