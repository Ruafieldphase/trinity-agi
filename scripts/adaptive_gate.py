...
class AdaptiveGate:
    ...
    def _make_decision(self,
        ...
        # --- Relational Meta-cognition Modulation [NEW] ---
        ...
        # Adjust threshold based on relational mode
        # Follower mode trusts the "Field" more, lowering the threshold for known domains
        if relational_state == "FOLLOWER":
            required_threshold = 0.4 + (domain_risk * 0.3) # 0.4 ~ 0.7
        elif relational_state == "ORCHESTRATOR":
            required_threshold = 0.6 + (domain_risk * 0.3) # 0.6 ~ 0.9
        else: # PIONEER
