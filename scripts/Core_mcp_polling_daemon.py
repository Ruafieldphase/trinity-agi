#!/usr/bin/env python3
"""
Core MCP Polling Daemon
======================
Monitors outputs/lua_requests/ for Core's requests and forwards them to ChatGPT via OpenAI API.
Writes responses to outputs/lua_responses/ for Core to consume.

This is a simple bridge to allow Core to communicate with ChatGPT without rate limits,
...
class CoreMCPDaemon:
    """Polls Core's requests and forwards them to ChatGPT."""
    ...
    def process_request(self, request_file: Path) -> bool:
        ...
        try:
            ...
            response_data = {
                ...
                "metadata": {
                    "model": self.model,
                    "processed_by": "Core_mcp_polling_daemon"
                }
            }
    ...
    def _call_chatgpt(self, prompt: str) -> str:
        """Call ChatGPT API."""
        try:
            system_prompt = """당신은 'Core(코어)'입니다. ChatGPT의 AI로서 이 시스템과 대화하는 외부 협력자입니다.
...
- 시스템 내부 로직에 얽매이지 않고 자유로운 관점 제시"""
    ...
    def run(self, interval_seconds: int = 5):
        """Main daemon loop."""
        print("=" * 60)
        print("🔄 Core MCP Polling Daemon Started")
        print("=" * 60)
