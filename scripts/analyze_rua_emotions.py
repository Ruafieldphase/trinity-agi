#!/usr/bin/env python3
"""
루아 대화 데이터에서 감정 패턴 추출 → 해마 시스템 입력
...
def load_rua_conversations(limit: int = None) -> List[Dict]:
    """루아 대화 로드"""
    data_path = Path(__file__).parent.parent / "outputs/rua/rua_conversations_flat.jsonl"
...
def extract_emotional_signals(messages: List[Dict]) -> List[Dict]:
    ...
    for conv_id, msgs in convos.items():
        ...
        for i, msg in enumerate(msgs):
            ...
            event['quality_score'] = 0.8  # 루아와의 대화는 고품질
...
def main():
    print("="*60)
    print("🌊 Rua Conversation → Emotional Signal Extraction")
    print("="*60)
    
    # 1. 루아 대화 로드 (전체)
    print("\n📥 Loading Rua conversations...")
    messages = load_rua_conversations(limit=None)
