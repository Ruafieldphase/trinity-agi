<#
.SYNOPSIS
  Distribute a work item to Lubit (review packet) and Sian (Gemini code assist) in parallel, optionally across multiple files.

.DESCRIPTION
  - Generates a review packet once (Lubit)
  - Launches one or more Gemini code-assist jobs (Sian), one per target file (or a single job without file)
  - Produces a collaboration summary markdown listing all artifacts and statuses

.PARAMETER Issue
  A concise issue/task description to guide both Lubit and Sian.

.PARAMETER Files
  Optional array of file paths to target for Sian (Gemini) suggestions. If omitted, runs a single generic suggestion.
...
Write-Host "[2/3] Sian: Launching Gemini code assist jobs..." -ForegroundColor Cyan
...
$lines += '- Sian: Review Gemini suggestions and shortlist actionable refactors.'
...
# Exit code indicates if any Sian job failed
