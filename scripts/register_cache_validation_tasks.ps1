...
function Register-Tasks {
    Write-Host "`n?îß Registering Cache Validation Scheduled Tasks..." -ForegroundColor Cyan
    ...
    foreach ($task in $Tasks) {
        Write-Host "`n?ìã Creating task: $($task.Name)" -ForegroundColor Yellow
        ...
        if ($existingTask) {
            if (-not $Force) {
                $response = Read-Host "   ?†Ô∏è  Task already exists. Overwrite? (y/N)"
                if ($response -ne 'y' -and $response -ne 'Y') {
                    Write-Host "   ??∏è  Skipped" -ForegroundColor Yellow
                    continue
                }
            }
            Unregister-ScheduledTask -TaskName $task.Name -Confirm:$false
            Write-Host "   ?óëÔ∏? Removed existing task" -ForegroundColor Gray
        }
    ...
    Write-Host "`n?ìÖ Schedule summary:" -ForegroundColor Cyan
...
function Unregister-Tasks {
    Write-Host "`n?óëÔ∏? Unregistering Cache Validation Scheduled Tasks..." -ForegroundColor Cyan
    ...
    foreach ($task in $Tasks) {
        ...
        if ($existingTask) {
            if (-not $Force) {
                ...
                if ($response -ne 'y' -and $response -ne 'Y') {
                    Write-Host "??∏è  Skipped: $($task.Name)" -ForegroundColor Yellow
                    continue
                }
            }
            ...
            Write-Host "?πÔ∏è  Not found: $($task.Name)" -ForegroundColor Gray
        }
    }
    ...
    if ($removedCount -gt 0) {
        ...
        Write-Host "`n?πÔ∏è  No tasks were removed" -ForegroundColor Gray
    }
}

function Show-Status {
    Write-Host "`n?ìä Current Cache Validation Tasks:" -ForegroundColor Cyan
    ...
    if (-not $foundAny) {
        Write-Host "`n?πÔ∏è  No cache validation tasks found" -ForegroundColor Gray
        Write-Host "?í° Run with -Register to create them" -ForegroundColor Yellow
    }
}
...
elseif ($Unregister) {
    ...
    Show-Status
    Write-Host "`n?í° Usage:" -ForegroundColor Yellow
