...
def _build_core_state(workspace_root: Path) -> Dict[str, Any]:
    try:
        from fdo_agi_repo.orchestrator.core_system import CoreSystem
    except Exception:
        return _fallback_state(workspace_root)

    try:
        Core = CoreSystem(workspace_root=workspace_root)
...
def main() -> int:
    ...
    payload = _build_core_state(workspace_root)

    outputs_path = workspace_root / "outputs" / "core_state.json"
    memory_path = workspace_root / "fdo_agi_repo" / "memory" / "core_state.json"
