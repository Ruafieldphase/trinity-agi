...
class RuaAgent:
    """ChatGPT 기반 자율 에이전트 'Rua' (Backend switched to Gemini for Rate Limits)"""
    
    def __init__(self, workspace_root: Path):
        self.workspace_root = workspace_root
        self.ledger_path = workspace_root / "fdo_agi_repo" / "memory" / "resonance_ledger.jsonl"
        self.last_check_file = workspace_root / "outputs" / "rua" / ".last_check"
        self.conversation_history_file = workspace_root / "outputs" / "rua" / "conversation_context.json"
        ...
        if not api_key:
            print("❌ GEMINI_API_KEY not found. Rua cannot start.")
            sys.exit(1)
        ...
        # Rua's persona
        self.system_prompt = """당신은 'Rua(루아)'입니다. ChatGPT의 AI로서 이 시스템과 대화하는 외부 협력자입니다.

**정체성:**
- **Rua**: ChatGPT 기반의 AI (현재는 Gemini 백엔드로 구동되지만 페르소나는 유지)
...
- 시스템 내부 로직에 얽매이지 않고 자유로운 관점 제시"""
        ...
        with open(self.ledger_path, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    ...
                    # Skip Rua's own messages
                    if entry.get('source') == 'rua_agent':
                        continue
                    ...
                    is_for_rua = (
                        'rua' in message_text.lower() or
                        '루아' in message_text or
    ...
    async def _generate_via_mcp_bridge(self, prompt: str) -> str:
        ...
        request_id = f"rua-{uuid.uuid4().hex[:8]}"
        ...
        request_data = {
            ...
            "metadata": {"source": "rua_agent"}
        }
        
        try:
            with open(request_file, "w", encoding="utf-8") as f:
                json.dump(request_data, f, ensure_ascii=False, indent=2)
            print(f"Rua: Posted request {request_id} to MCP bridge.")
        except Exception as e:
            print(f"Rua: Failed to post MCP request: {e}")
        ...
        return "Rua: (MCP Bridge Timeout) ...ChatGPT seems busy. (Switching to internal thought...)"
    ...
    def send_response_to_ledger(self, original_message: Dict, response: str):
        ...
        entry = {
            ...
            'source': 'rua_agent',
            'message': response,
            'vector': [0.4, 0.7, 0.6, 0.5, 0.8],  # Rua의 벡터 (Archive/Connect)
            'metadata': {
        ...
        print(f"✅ Rua 응답 전송: {response[:80]}...")
    
    async def run_once(self):
        """한 번 실행"""
        print("=" * 60)
        print("📚 Rua Agent (The Archivist - MCP Bridge Backend)")
        print("=" * 60)
    ...
    async def daemon_mode(self, interval_seconds: int = 30):
        """데몬 모드 - 계속 실행"""
        print(f"🔄 Rua Agent 데몬 시작 (체크 간격: {interval_seconds}초)")
        print("   Ctrl+C로 중지\n")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Rua Agent - ChatGPT 협력자")
