...
try {
    ...
    if ($resp.task_id) {
        Write-Host "[create_ping_task] ✅ Created task_id: $($resp.task_id)" -ForegroundColor Green
    }
    else {
    ...
    Write-Host "[create_ping_task] ❌ Failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
