...
def compute_drives(now: float) -> tuple[float, float, DriveVector, dict[str, Any]]:
    ...
    # Check Ledger for recent autonomous play (Satiation override)
    # If autonomous_play happened recently, exploration age is effectively low
            ...
            with open(ledger_path, "r", encoding="utf-8") as f:
                ...
                for line in reversed(lines[-20:]):
                    try:
                        ...
                        if entry.get("type") == "autonomous_play":
                           ts_str = entry.get("timestamp")
                           # Simple ISO parse (robust enough for local)
                           # e.g. 2026-01-06T11:20:57
                           # If timezone issues arise, we accept rough diff
                           # But simple check: if we found it, assume it's recent if near end of file?
    ...
    # Use the more recent of Explicit Exploration or Autonomous Play
    if play_age is not None and (explore_age is None or play_age < explore_age):
        explore_age = play_age
