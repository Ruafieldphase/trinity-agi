...
# -*- coding: utf-8 -*-
"""
Core Conversation Intake (Hippocampus-friendly index)

목표:
- `ai_binoche_conversation_origin/Core`의 대화/사유 기록을
  무거운 분석 없이 "인덱스(메타 + 짧은 발췌 + 경계 후보)"로 고정한다.
- 이 인덱스는 full_cycle 리포트와 해마(episodic) 전사의 입력이 된다.

입력:
- <workspace>/ai_binoche_conversation_origin/Core/*.md (주로)
- (옵션) origin/conversations.json (있으면 존재만 기록)

출력:
- <workspace>/outputs/core_conversation_intake_latest.json
- <workspace>/outputs/core_conversation_intake_history.jsonl (append)
"""
...
def _load_prev_latest(workspace_root: Path) -> Dict[str, Any]:
    p = workspace_root / "outputs" / "core_conversation_intake_latest.json"
    if not p.exists():
        return {}
    try:
...
def run_core_conversation_intake(workspace_root: Path, max_docs: int = 120) -> Dict[str, Any]:
    workspace_root = workspace_root.resolve()
    base = workspace_root / "ai_binoche_conversation_origin" / "Core"
    ...
    return {
        ...
        "note": f"docs에는 최근 {max_docs}개만 포함(요약). 원본은 ai_binoche_conversation_origin/Core에 존재.",
    }
...
def main() -> int:
    ...
    ap.add_argument("--out", type=str, default=str(Path("outputs") / "core_conversation_intake_latest.json"))
    ap.add_argument("--history", type=str, default=str(Path("outputs") / "core_conversation_intake_history.jsonl"))
    ...
    result = run_core_conversation_intake(ws, max_docs=int(args.max_docs))
