...
def _get_adaptive_cooldown(base_sec: int = 900) -> int:
    """리듬 기반 adaptive cooldown"""
    # 1. 리듬 지표 로드 (rhythm_health_latest.json)
    health_path = WORKSPACE / "outputs" / "rhythm_health_latest.json"
    rhythm_mode = "NEUTRAL"
    if health_path.exists():
        try:
            health = json.loads(health_path.read_text(encoding='utf-8'))
            score = health.get("score", 50)
            # 60 기준 (rhythm_think.py 준수)
            rhythm_mode = "EXPANSION" if score > 60 else "CONTRACTION"
        except: pass

    rhythm_factor = {
        "EXPANSION": 0.3,     # 4.5분 (빠른 학습)
        "CONTRACTION": 1.5,   # 22.5분 (느린 학습)
        "NEUTRAL": 1.0        # 15분 (기본)
    }.get(rhythm_mode, 1.0)

    return int(base_sec * rhythm_factor)

def _cooldown_ok(now: float, cooldown_sec: int = 900) -> bool:
    st = _load_state()
    ...
    adaptive_sec = _get_adaptive_cooldown(base_sec=cooldown_sec)
    
    if last_f and (now - last_f) < float(adaptive_sec):
        return False
    return True
