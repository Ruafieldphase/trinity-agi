...
class HumanSummaryGenerator:
    ...
    def _extract_tags(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 소스 기반 공간 위상 추론
        # - trigger_report_latest.json에서 sources 위치가 실행 형태에 따라 달라질 수 있으므로,
        #   full_cycle 포함 케이스도 안전하게 파생한다.
        result_summary = data.get("result_summary", {}) or {}
        compressed = result_summary.get("compressed", result_summary) if isinstance(result_summary, dict) else {}
        sources = []
        if isinstance(compressed, dict) and isinstance(compressed.get("sources"), list):
            sources = list(compressed.get("sources") or [])

        # Exploration/OBS Recode는 self_acquire 결과에서 파생되는 소스이므로 보강한다.
        try:
            sa = result_summary.get("self_acquire", {}) if isinstance(result_summary, dict) else {}
            if isinstance(sa, dict):
                if (sa.get("exploration_intake") or {}).get("ok") and "exploration" not in sources:
                    sources.append("exploration")
                obs = sa.get("obs_recode_intake") if isinstance(sa.get("obs_recode_intake"), dict) else {}
                if isinstance(obs, dict) and (obs.get("total") or 0) > 0 and "obs_recode" not in sources:
                    sources.append("obs_recode")
        except Exception:
            pass
        ...
        elif "exploration" in sources:
            tags["spatial_context"] = "능동 탐색 (Exploration)"
        elif "obs_recode" in sources:
            tags["spatial_context"] = "행동 학습 (OBS Recode)"
        else:
    ...
    def generate_from_trigger_report(self, report_data: Optional[Dict[str, Any]] = None) -> HumanSummary:
        ...
        # Exploration 소스 추가 확인
        if report_data.get("result_summary", {}).get("self_acquire", {}).get("exploration_intake", {}).get("ok"):
            if "exploration" not in sources:
                sources.append("exploration")
        
        # OBS Recode 소스 추가 확인 (행동/조작 데이터)
        obs_idx = report_data.get("result_summary", {}).get("self_acquire", {}).get("obs_recode_intake", {})
        if isinstance(obs_idx, dict) and (obs_idx.get("total") or 0) > 0:
            if "obs_recode" not in sources:
                sources.append("obs_recode")
        ...
        # 경계 학습 상태(있으면): boundary_map_latest.json 또는 report result_summary에서 파생
        try:
            bm = (report_data.get("result_summary") or {}).get("boundary_map", {})
            if isinstance(bm, dict):
                stats = bm.get("stats") if isinstance(bm.get("stats"), dict) else {}
                total = (stats.get("total_rules") if isinstance(stats, dict) else None)
                if isinstance(total, int) and total > 0:
                    tags["boundary_learning"] = f"경계 {total}개"
        except Exception:
            pass

        # 내부 상태 로드
        internal_state_data = self._load_json(self.internal_state) or {}
        # 누적 압축 이벤트(총량): "compression_count=0"이 '학습이 멈춤'으로 오해되는 것을 방지하기 위해
        # internal_state의 누적치를 함께 사용한다.
        try:
            compression_events_total = int(
                (internal_state_data.get("self_expansion") or {}).get("compression_events") or 0
            )
        except Exception:
        ...
        internal_snapshot = {
            ...
            "compression_events_total": compression_events_total,
        }
        ...
        return HumanSummary(
            ...
            # full_cycle/self_compress에서는 count가 의미가 있지만, heartbeat_check 등에서는 0이 일반적이다.
            # 이때는 누적치(total)를 사용해 "소화/압축이 0"이라는 오해를 줄인다.
            compression_count=count if int(count or 0) > 0 else int(compression_events_total or 0),
