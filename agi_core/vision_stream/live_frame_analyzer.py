...
class LiveFrameAnalyzer:
    ...
    async def analyze_frame(self, frame: VisionFrame) -> Dict[str, Any]:
        ...
        prompt_path = Path(__file__).parent.parent.parent / "prompts" / "vision_rhythm.txt"
        if prompt_path.exists():
            prompt_template = prompt_path.read_text(encoding="utf-8")
        else:
            prompt_template = "Analyze the screen rhythm and return JSON." # Fallback

        try:
            image_part = Part.from_data(data=frame.data, mime_type="image/jpeg")
            response = await asyncio.to_thread(
                self.model.generate_content,
                [image_part, prompt_template]
            )
