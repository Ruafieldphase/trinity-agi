...
class RhythmBoundaryManager:
    def __init__(self, workspace_root: Path):
        ...
        self.state_dir = workspace_root / "outputs" / "governance"
        self.state_dir.mkdir(parents=True, exist_ok=True)
        
        self.phase_boundary = BreathingBoundary(base=60, name="phase", behavior="higher_is_safer")
        self.phase_boundary.load_state(str(self.state_dir / "boundary_phase.json"))
        
        self.last_phase = "CONTRACTION"
        self.optimizer = SelfOptimizer(workspace_root)
        ...
        try:
            ...
            if health_path.exists():
                with open(health_path, "r", encoding="utf-8") as f:
                    ...
                    # Breathing Boundary logic
                    context = {
                        "rhythm_mode": "NEUTRAL", # Phase manager itself doesn't check mode yet
                        "current_state": "inside" if self.last_phase == "EXPANSION" else "outside",
                        "velocity": 0, # Velocity not tracked here yet
                        "trust": 1.0
                    }
                    threshold = self.phase_boundary.get_threshold(context)
                    state["phase"] = "EXPANSION" if score > threshold else "CONTRACTION"
    ...
    def perform_self_tuning(self):
        """
        경험(Mimesis)을 바탕으로 경계를 스스로 최적화합니다.
        """
        performance = self.optimizer.analyze_modeling_performance()
        if not performance:
            return False

        tuning_signals = self.optimizer.calculate_tuning_signal(performance)
        
        # Phase Boundary 개선
        phase_adj = tuning_signals.get("phase_base_adjustment", 0.0)
        if phase_adj != 0:
            self.phase_boundary.tune(phase_adj)
            self.phase_boundary.save_state(str(self.state_dir / "boundary_phase.json"))
            
            # Log to optimization log
            self.optimizer.log_optimization(tuning_signals, performance)
            return True
