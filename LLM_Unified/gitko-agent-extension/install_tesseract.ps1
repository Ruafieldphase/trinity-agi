...
Write-Host "🔍 Tesseract OCR 설치 시작..." -ForegroundColor Cyan
...
if (Test-Path $tesseractExe) {
    Write-Host "✅ Tesseract OCR이 이미 설치되어 있습니다!" -ForegroundColor Green
...
# 5. 자동 설치 (무인 설치)
Write-Host "🚀 Tesseract OCR 자동 설치 중..." -ForegroundColor Cyan
...
try {
    ...
    # 설치 확인
    if (Test-Path $tesseractExe) {
        Write-Host "✅ Tesseract OCR 설치 완료!" -ForegroundColor Green
        Write-Host "   경로: $tesseractExe" -ForegroundColor Gray
        & $tesseractExe --version
        ...
        if ($currentPath -notlike "*$installPath*") {
            ...
            Write-Host "✅ PATH 추가 완료 (재부팅 후 적용)" -ForegroundColor Green
        }
        
        # Python 스크립트 업데이트
        $pythonScript = "D:\nas_backup\LLM_Unified\ion-mentoring\computer_use.py"
        if (Test-Path $pythonScript) {
            Write-Host "📝 computer_use.py 업데이트 중..." -ForegroundColor Yellow
            ...
            Write-Host "✅ Python 스크립트 업데이트 완료" -ForegroundColor Green
        }
