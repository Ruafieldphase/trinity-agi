...
if (-not $env:SLACK_BOT_TOKEN) {
    Write-Host "⚠️  SLACK_BOT_TOKEN not set!" -ForegroundColor Yellow
...
if ($KillExisting) {
    Write-Host "🔍 Checking for existing Slack bot processes..." -ForegroundColor Cyan
...
Write-Host "🚀 Starting Ion Canary Slack Bot on port $Port..." -ForegroundColor Green
...
try {
    ...
    if ($response.status -eq "ok") {
        Write-Host "✅ Slack bot started successfully!" -ForegroundColor Green
    ...
    Write-Host "❌ Failed to verify bot health check" -ForegroundColor Red
