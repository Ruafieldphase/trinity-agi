...
def load_Core_data(jsonl_path: Path) -> List[Dict[str, Any]]:
    """Load and filter Core dataset"""
    conversations = []
...
def main():
    ...
    Core_path = outputs_dir / 'core_dataset_parsed.jsonl'
    Core_data = load_Core_data(Core_path)
    print(f"  Loaded {len(Core_data)} Core messages")
