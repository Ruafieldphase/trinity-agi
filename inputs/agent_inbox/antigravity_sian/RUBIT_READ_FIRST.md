...
## 2) 자동 공유(Shion이 자동으로 받는 것)
- `scripts/coordination/publish_ops_snapshot.py`가 `inputs/agent_inbox/antigravity_shion/OPS_SNAPSHOT.md`로 스냅샷을 best-effort로 갱신한다.
...
## 5) 참고: Shion Phase 1~3 종합 보고서에 대한 사실 검증
- `outputs/agent_outbox/antigravity_shion/SUMMARY_SHION_BIOLOGICAL_PHASES_FACTCHECK.md`
- 목적: “확인됨 / 부분 구현 / 미검증”을 분리해서 과대 주장으로 운영 판단이 흔들리지 않게 하기 위함.
