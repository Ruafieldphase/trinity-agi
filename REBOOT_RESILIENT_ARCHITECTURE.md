...
graph TD
    A[Windows 부팅] --> B[로그온]
    B --> C[Master Orchestrator 자동 실행]
    C --> D[업그레이드 체크]
    D --> E[Task Queue Server 시작]
    E --> F[RPA Worker 시작]
    F --> G[Monitoring Daemon 시작]
    G --> H[Watchdog 시작]
    H --> I[시스템 상태 검증]
    I --> J{모두 정상?}
    J -->|Yes| K[✅ 준비 완료]
    ...
    M[VS Code 열기] --> N[Auto Bring-up Task]
    N --> O[백그라운드 프로세스 확인]
    O --> P{누락 있음?}
    P -->|Yes| Q[자동 시작]
    P -->|No| R[대시보드 업데이트]
...
```mermaid
graph LR
    A[Git Pull] --> B[커밋 해시 변경 감지]
    B --> C[requirements.txt 변경?]
    C -->|Yes| D[pip install]
    C -->|No| E[package.json 변경?]
    E -->|Yes| F[npm install]
    E -->|No| G[새 스크립트?]
    G -->|Yes| H[권한 설정]
    D --> I{프로세스 재시작 필요?}
    F --> I
    H --> I
    I -->|Yes| J[Master Orchestrator 재시작]
    I -->|No| K[✅ 완료]
    J --> K
```
